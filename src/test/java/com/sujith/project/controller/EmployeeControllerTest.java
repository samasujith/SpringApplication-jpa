...
class EmployeeControllerTest {
    ...
    @Test
    @DisplayName("deleteEmployeeById_valid")
    void deleteEmployeeById_valid() throws Exception {
        when(employeeService.delete(1)).thenReturn(EmployeeDtoObject.getEmp());
        mockMvc.perform(MockMvcRequestBuilders.delete("/api/employee/1")).andExpect(status().isOk()).andExpect(result -> {
            EmployeeDtoObject.getEmp();
        });
    }

    @Test
    @DisplayName("deleteEmployeeById_throwInvalidDataAccessApiUsageException")
    void deleteEmployeeById_throwInvalidDataAccessApiUsageException() throws Exception {
        when(employeeService.delete(1)).thenThrow(new InvalidDataAccessApiUsageException("Employee does not exist"));
        mockMvc.perform(MockMvcRequestBuilders.delete("/api/employee/1"))
                .andExpect(status().isBadRequest()).andExpect(jsonPath("$.message").value("Employee does not exist"));
    }
