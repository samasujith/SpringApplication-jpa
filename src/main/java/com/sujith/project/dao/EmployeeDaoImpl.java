...
public class EmployeeDaoImpl implements EmployeeDao {
    ...
    public List<Employee> findAll() {
        TypedQuery<Employee> thedata = entityManager.createQuery("from Employee ", Employee.class);
        if (thedata.getResultList().isEmpty()) {
            logger.warn("Database is Empty !");
            throw new ApiRequestException("No Employees were found in Database");
        } else {
            return thedata.getResultList();
        }
    }
    ...
    public Employee delete(int id) {
        Employee tempEmp = findById(id);
        Employee tempEmp1 = findById(id);
